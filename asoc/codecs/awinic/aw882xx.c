...
static struct mutex g_msg_dsp_lock;
...
static int aw_misc_ops_msg(struct aw882xx *aw882xx, unsigned long arg)
{
	...
	if(ioctl_msg.version != AW_IOCTL_MSG_VERSION) {
		pr_err("unsupported msg version %d", ioctl_msg.version);
		return -EINVAL;
	}
	...
	if (ioctl_msg.type == AW_IOCTL_MSG_RD_DSP) {
		...
		pr_err("unsupported msg type %d", ioctl_msg.type);
		return -EINVAL;
	}
}
