...
static uint8_t fade_status = 0; // function switch
static uint8_t is_fading = 0;
static uint8_t is_need_fade = 0; // speaker or not
static uint8_t g_step = 17;
static uint8_t mute_status = 1; // control by tfa98xx_mute
struct task_struct *fade_thrd;
unsigned char tfa98xx_volume_tab[33] = /* index 0 is sleep time, start from index 1 */
{
...
int __attribute__((weak)) mtk_spk_send_ipi_buf_to_dsp(void *data_buffer, uint32_t data_size)
{
	(void)data_buffer;
	(void)data_size;
    pr_err("[%s]you need to define this func in other module!\n", __FUNCTION__);
...
int __attribute__((weak))  mtk_spk_recv_ipi_buf_from_dsp(int8_t *buffer, int16_t size, uint32_t *buf_len)
{
	(void)buffer;
	(void)size;
	(void)buf_len;
...
#ifdef CONFIG_MTK_PLATFORM
	if((strstr(tfa_cont_profile_name(tfa98xx, prof_idx), "music") != NULL) ||
        (strstr(tfa_cont_profile_name(tfa98xx, prof_idx), "speaker") != NULL)) {
		is_need_fade = 1;
	}else{
		is_need_fade = 0;
	}
...
#ifdef CONFIG_MTK_PLATFORM
static int tfa98xx_info_algo_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_BOOLEAN;
	mutex_lock(&tfa98xx_mutex);
...
uint8_t algo_status = 1;
static int tfa98xx_get_algo_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	mutex_lock(&tfa98xx_mutex);
	ucontrol->value.integer.value[0] = algo_status;
	mutex_unlock(&tfa98xx_mutex);

	return 0;
}

static uint8_t algo_bytes[3*2] = {0};
static int tfa98xx_set_algo_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx;
	int error = 0;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tfadsp_volume >=0 && tfadsp_volume <= 255)
		tfa98xx_send_volume(tfadsp_volume, 1); // 1 means only change left channel vol.
	else
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	scnprintf(name, MAX_CONTROL_NAME, "%s Algo", tfa98xx->fw.name);
	tfa98xx_controls[mix_index].name = name;
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	if (tfa98xx->tfa->is_probus_device == 1)
		tfa98xx->tfa->has_msg = 1;                          //modify by mono for non dsp 20231111
	else
...
static int tfa98xx_fade_thread(void *data)
{
	...
	for(times = 1; times < g_step; times++) {
	    if (mute_status ==1) {
            ...
            if (tfadsp_volume > tfa98xx_volume_tab[times])
                vol_tmp = tfadsp_volume;
            else
                vol_tmp = tfa98xx_volume_tab[times];

			tfa98xx_send_volume(vol_tmp, 1);
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK)
		{
    		...
    		if (tfa98xx->tfa->is_probus_device) {
                if (tfa98xx->tfa->is_otp_device) {
        			...
        			pr_debug("send_calib_values from MTP\n");
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	{
        ...
        .symmetric_rate = 1,                    //modify by mono for kernel6.1 20231030
#else
        .symmetric_rates = 1,
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,12,0)
        .symmetric_sample_bits = 1,             //modify by mono for kernel6.1 20231030
#else
	...
	char *buf, loff_t off, size_t count)
{
	...
	pr_debug("P cal data is: 0x%x\n", tfa98xx_cali_l);
	pr_debug("S cal data is: 0x%x\n", tfa98xx_cali_r);
...
/*
	...
	cali_data[6] = (uint8_t)((cali_r >> 16) & 0xff);
	...
	mutex_lock(&tfa98xx->dsp_lock);
	if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
		error = tfa98xx_send_data_to_dsp(cali_data, sizeof(cali_data));
		pr_debug("P cal data is: %x%x%x\n", cali_data[3],cali_data[4],cali_data[5]);
		pr_debug("S cal data is: %x%x%x\n", cali_data[6],cali_data[7],cali_data[8]);
	} else {
		pr_err("DSP is Not ready\n");
		error = -ENODEV;
	}
	if (error != Tfa98xx_Error_Ok) {
		pr_debug("[0x%x] dsp_msg error: %d\n", tfa98xx->i2c->addr, error);
...
};*/                                        //modify by mono for rpc climax  need fix
...
#endif
{
	...
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs vol files\n");
#endif
...
#else
static int tfa98xx_i2c_remove(struct i2c_client *i2c)           //modify by mono for kernel6.1 20231030
#endif
{
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
...
//	device_remove_bin_file(&i2c->dev, &dev_attr_rpc);    //modify by mono for rpc climax
	...
	if (gpio_is_valid(tfa98xx->reset_gpio))
		devm_gpio_free(&i2c->dev, tfa98xx->reset_gpio);    //modify by mono for kernel6.1 20231030
#endif
