...
static enum Tfa98xx_Error tfa986x_specific(struct tfa_device *tfa)
{
	...
	case 0x100a66:/**TFA9866 N2A0**/
		...
		tfa_reg_write(tfa, 0x5c, 0xb02b); //POR=0xde96
		tfa_reg_write(tfa, 0x5f, 0x00a0); //POR=0x00c0
		tfa_reg_write(tfa, 0x62, 0x06c4); //POR=0x0582
		tfa_reg_write(tfa, 0x63, 0x80d4); //POR=0x0602
		tfa_reg_write(tfa, 0x65, 0x0c58); //POR=0x0458
		tfa_reg_write(tfa, 0x67, 0x006e); //POR=0x0602
		tfa_reg_write(tfa, 0x68, 0x0820); //POR=0x0c20
	...
	case 0x200a66:/**TFA9866 N3A0**/
		rc = 0;
		rc = tfa_get_bf(tfa, 0xf0a5);
		if (rc <  0) {
			return rc;
		}
		value = (uint16_t)rc;
		tfa_set_bf(tfa, 0x8305, value);

		/* ----- generated code start(V6) ----- */
		/* -----  version 12 ----- */
		tfa_reg_write(tfa, 0x08, 0x009a); //POR=0x00d2
		tfa_reg_write(tfa, 0x50, 0xc000); //POR=0x8000
		tfa_reg_write(tfa, 0x62, 0x0666); //POR=0x06c6
		tfa_reg_write(tfa, 0x63, 0x806d); //POR=0x80d4
		tfa_reg_write(tfa, 0x65, 0x0c58); //POR=0x0458
		tfa_reg_write(tfa, 0x67, 0x016d); //POR=0x0628
		tfa_reg_write(tfa, 0x74, 0x5e28); //POR=0x6014
		tfa_reg_write(tfa, 0x75, 0x1daa); //POR=0x39e0
		/* ----- generated code end   ----- */
		break;
	...
	pr_err("Info: found  device with revid = 0x%x \n", tfa->revid);
